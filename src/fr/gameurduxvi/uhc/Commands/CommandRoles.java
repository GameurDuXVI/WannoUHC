							...
							for(Personnage pers: Main.getInstance().getPersonages()) {
								if(pers.getId() == id) {
									...
									if(pd.getPersonnage().getType() == 2) {
										...
										p.sendMessage("§7Roles §8>> §cPour changer de fidèlité, il faut donner le role, puis mettre la fidèlité");
										return false;
									}
									...
									if(p.equals(target)) {
										p.sendMessage("§7Roles §8>> §aLa fidèlité a bien été mis à vous même");
									}
									else {
										p.sendMessage("§7Roles §8>> §aLa fidèlité a bien été mis à §b" + target.getName());
									}
									target.sendMessage("§7Roles §8>> §bVotre fidèlité a changé envers §b" + pers.getName());
									return false;
								}
							}
						}
